...
using ShopsService.Domain.MongoDb.Abstract;
using System.Linq.Expressions;

namespace ShopsService.Domain.MongoDb.Concrete
{
    public class MongoRepository<T> : IMongoRepository<T> where T : BaseEntity, new()
    {
        protected readonly IMongoCollection<T> Collection;
        private readonly DatabaseSetting settings;

        public MongoRepository(IOptions<DatabaseSetting> options)
        {
        ...
        {
            return Collection.Count(x => !x.IsDeleted);
        } 
    }
}
